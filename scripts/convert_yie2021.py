...
import io,os,re,sys
root='.'
html_path=os.path.join(root,'yie2021.html')
if not os.path.exists(html_path):
    print('yie2021.html not found',file=sys.stderr); sys.exit(1)
...
for m in pattern.finditer(s,start):
    if m.group(1)=='':
        ...
        count-=1
    # once we've closed the initial div and count==0, capture until end of this match's end
    if count==0:
        end_pos = m.end()
        break
else:
...
article_html=article_html.replace('./yie2021_files/','./')
# pick a cover image filename if present
# try to find first img src inside article_html
...
fm = []
fm.append('+++')
fm.append('title = "The Year in Ethereum 2021"')
fm.append('date = 2022-01-17T00:00:00')
fm.append('description = "The Year in Ethereum 2021"')
fm.append('tags = ["year-in-ethereum", "ethereum"]')
fm.append('draft = false')
if cover:
    fm.append('\n[params.cover]')
    fm.append(f'image = "{cover}"')
    fm.append('alt = "The Year in Ethereum 2021"')
    fm.append('caption = "The Year in Ethereum 2021"')
fm.append('+++\n')
content='\n'.join(fm)+article_html+"\n"
out_dir=os.path.join(root,'content','posts','YIE2021')
...
assets_src=os.path.join(root,'yie2021_files')
if os.path.exists(assets_src):
    ...
    # remove the now-empty assets_src directory if possible
    try:
        os.rmdir(assets_src)
    ...
    print('assets folder not found; remember to copy yie2021_files into',out_dir)
print('Done')
